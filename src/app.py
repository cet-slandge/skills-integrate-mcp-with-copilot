...
def _require_teacher(authorization: str | None) -> str:
    """Raises 401 if the Authorization header is missing or invalid."""
...
def login(body: LoginRequest):
    ...
    hashed = TEACHERS.get(body.username)
    if not hashed or not bcrypt.checkpw(body.password.encode(), hashed.encode()):
        raise HTTPException(status_code=401, detail="Invalid username or password")
    token = secrets.token_hex(32)
    active_sessions[token] = body.username
...
def get_me(authorization: str | None = Header(default=None)):
    """Return the currently logged-in teacher username, or null."""
    if authorization and authorization.startswith("Bearer "):
        token = authorization.removeprefix("Bearer ")
